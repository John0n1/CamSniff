...
REQ_HASH_FILE="$SCRIPT_DIR/.req_hash"
USE_VENV=1
...
if [[ -f "$REQ_FILE" ]]; then
  if (( USE_VENV )); then
    retry pip install --no-cache-dir -r "$REQ_FILE" --quiet
  else
    retry pip3 install --user --no-cache-dir -r "$REQ_FILE" --quiet || true
  fi
  # Record hash after successful install (best-effort)
  sha256sum "$REQ_FILE" 2>/dev/null | awk '{print $1}' > "$REQ_HASH_FILE" 2>/dev/null || true
else
  if (( USE_VENV )); then
    retry pip install --no-cache-dir wsdiscovery opencv-python requests flask aiohttp fastapi uvicorn pre-commit ruff black typer httpx --quiet
  else
    retry pip3 install --user --no-cache-dir wsdiscovery opencv-python requests flask aiohttp fastapi uvicorn pre-commit ruff black typer httpx --quiet || true
  fi
fi
