...
# Idempotency guard
if [[ -n "${_CAMSNIFF_ENV_SETUP_RAN:-}" ]]; then
  return 0 2>/dev/null || exit 0
fi
_CAMSNIFF_ENV_SETUP_RAN=1
...
log_debug "Starting env_setup.sh (idempotent)"
...
SS=$(JQ -r '.sleep_seconds' "$CONFIG_FILE") || { log_debug "Failed to load sleep_seconds"; SS=60; }
...
if [[ "$RTSP_LIST_URL" == *"CamioCam/rtsp"* ]]; then
  ...
  RTSP_LIST_URL="https://github.com/John0n1/CamSniff/blob/4d682edf7b4512562d24ccdf863332952637094d/src/rtsp_paths.csv"
fi
...
# Fallback for legacy configs with src/ prefix
if [[ "$PASSWORD_WORDLIST" == src/* ]]; then
  log_debug "Legacy password_wordlist detected, switching to passwords.txt"
  PASSWORD_WORDLIST="passwords.txt"
fi
...
# Fallback for legacy configs with src/ prefix
if [[ "$USERNAME_WORDLIST" == src/* ]]; then
  log_debug "Legacy username_wordlist detected, switching to usernames.txt"
  USERNAME_WORDLIST="usernames.txt"
fi
...
# Always create a unique SNMP community file (avoid permission issues with stale file)
SNMP_COMM_FILE=$(mktemp /tmp/.snmp_comms.XXXXXX)
if ! printf "%s\n" "${SNMP_COMM_ARRAY[@]}" > "$SNMP_COMM_FILE" 2>/dev/null; then
  log_debug "Failed to write SNMP_COMM_FILE at $SNMP_COMM_FILE"
  exit 1
fi
chmod 600 "$SNMP_COMM_FILE" 2>/dev/null || true
export SNMP_COMM_FILE
log_debug "SNMP_COMM_FILE created at $SNMP_COMM_FILE (permissions hardened)"
...
export CAMSNIFF_OUTPUT="${CAMSNIFF_OUTPUT:-$(dirname "$(dirname "${BASH_SOURCE[0]}")")/output}"
