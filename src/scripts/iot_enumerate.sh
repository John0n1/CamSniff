...
# Provide phase() fallback if not already defined (so instrumentation is consistent)
if ! type phase >/dev/null 2>&1; then
  phase(){ echo "=== PHASE: $* ==="; };
fi

safe_counts(){
  local cam dev streams
  cam=$( (declare -p CAMERAS_FOUND 2>/dev/null | grep -q 'declare -A') && echo ${#CAMERAS_FOUND[@]} || echo 0 )
  dev=$( (declare -p DEVICE_INFO 2>/dev/null | grep -q 'declare -A') && echo ${#DEVICE_INFO[@]} || echo 0 )
  streams=$( (declare -p STREAMS 2>/dev/null | grep -q 'declare -A') && echo ${#STREAMS[@]} || echo 0 )
  echo "[SUMMARY] Cameras=${cam} Devices=${dev} Streams=${streams}"
}
...
scan_wireless_cameras(){
  ...
  local -a OUIS
  if [[ -f "$SCRIPT_DIR/rtsp_paths.csv" ]]; then
    mapfile -t OUIS < <(awk -F',' 'NR>1 {print $4}' "$SCRIPT_DIR/rtsp_paths.csv" | tr -d '"' | tr '|' '\n' | sed 's/[() ]//g' | grep -E '^[0-9A-Fa-f]{2}(:[0-9A-Fa-f]{2}){2}$' | sort -u | head -n 100)
  fi
  local oui_regex
  if ((${#OUIS[@]})); then
...
iot_enumeration_cycle(){
  ...
  phase "IOT ENUMERATION" "mDNS / SSDP / WiFi / BLE / Zigbee/Z-Wave / Custom Protocols / Topology"
  start_pcap_capture
  phase "mDNS"
  enumerate_mdns
  safe_counts
